        ...
        <div>
          <div class="search">
            <el-select v-model="searchType" placeholder="请选择">
              ...
              <el-option label="加氢站" value="2"></el-option>
            </el-select>
            <el-input type="text" v-model="searchName" placeholder="检索条件"></el-input>
            <el-button class="input-in-btn" @click="searchMarker" icon="el-icon-search"></el-button>
          </div>
          <div class="checkBox">
            <el-radio-group v-model="radioType" @change="radioTypeChange">
              <el-radio :label="'3'">全部</el-radio>
              <el-radio :label="'1'">车辆</el-radio>
              <el-radio :label="'2'">加氢站</el-radio>
            </el-radio-group>
          </div>
...
export default {
  ...
  methods: {
    ...
    setVehicleSituation(marker, vehicle) {
      if (marker.getExtData().status !== vehicle.status) {
        let icon = null;
        if (vehicle.mpa === "MPA35" && item.status === "1") {
          icon = new AMap.Icon({
            size: new AMap.Size(53, 65), // 图标尺寸
            image: require("@/assets/img/statistics/vehicle35-map.png"), // Icon的图像
            imageSize: new AMap.Size(53, 65), // 根据所设置的大小拉伸或压缩图片
        ...
        if (vehicle.mpa === "MPA70" && item.status === "1") {
          icon = new AMap.Icon({
            size: new AMap.Size(53, 66), // 图标尺寸
            image: require("@/assets/img/statistics/vehicle70-map.png"), // Icon的图像
            imageSize: new AMap.Size(53, 66), // 根据所设置的大小拉伸或压缩图片
    ...
    newVehicleVueComponentWindow(item, position, marker) {
      ...
      <img src="${this.location}" alt /><span>{{vehicleInfo.vehicleNum}} （{{vehicleInfo.mpa=="MPA70"?"70MPa":"35MPa"}}）</span>
           <div class="vehicleTrack">
          <el-button @click="goBicle">车辆跟踪</el-button>
        </div>
      </div>
    ...
    newSiteVueComponentWindow(item, position, marker) {
      ...
      <div class="title">
      <img src="${this.site}" alt /><span>站点名称：{{siteInfo.tenantName}}({{siteInfo.mpa=="MPA70"?"70MPa":"35MPa"}}）</span><div class="vehicleTrack">
          <el-button @click="goHydrogenationStation">加氢站详情</el-button>
        </div></div>
      <div class="infoWindowContent">
        <img class="qrCode" src="${this.qrCode}" alt />
        <div class="contentText">
  ...
  created() {
    this.getTree({ dataType: this.radioType });
  },
  mounted() {
    this.mapInit();
