<template>
  <div class="flexBlock">
    <div class="content">
      <!--上半部分-->
      <div style="margin:0.2% 0 0 0;display: flex;justify-content: center;height:46.5%">
          ...
          <div class="one">
            ...
            <div style="height:53%;margin-top: 3%" ref="work">
              <div class="workHeight" v-for="workItem in dataWork">
                <span class="point">{{workItem.nameShort}}（由{{workItem.assigneeName}}发起）</span><span class="newsHeightDate">{{workItem.createTime}}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        this.$store.state.menu.forEach((item) => {
          if (item.name != "首页") {
            item.children.forEach((f) => {
              if ('/industryService/companyEntry' == f.node.path) {
                sessionStorage.setItem("dropdownFather", item.name);
                sessionStorage.setItem("dropdownSon", f.name);
                this.$store.state.dropdown = { father: item.name, son: f.name };
              }
            });
          }
        });
...
<style scoped>
  ...
  @media screen and (max-width: 1600px) {
    ...
    /deep/.el-calendar-table /deep/.el-calendar-day {
      font-size: 12px;
    }
