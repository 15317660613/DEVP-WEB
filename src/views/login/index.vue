        ...
        >
          ...
          <el-form-item>
            <el-row>
              ...
              <el-col :span="12">
                <span class="linkButtton" @click="forgetPWD" style="float: right;">忘记密码</span>
              </el-col>
            </el-row>
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods: {
    ...
    loginHttp () {
      ...
      this.$http.post('/login', userInfo).then(({ data }) => {
        ...
        if (data.code == '200') {
          ...
          this.$store.commit('changeTenantName', data.data.tenantName)
          this.$store.commit('changeTenantId', data.data.tenantId)
          this.$store.commit('changeTenantType', data.data.tenantType)
    ...
    goR () {
      ...
      var myurl = window.location.origin + '/devWeb/#/views/Register'
      // var myurl= './Register'
      window.open(myurl, '_self')
    },
    forgetPWD(){
      this.$alert('请联系管理员，重置密码！', '提示信息', {});
    }
  }
}
</script>
