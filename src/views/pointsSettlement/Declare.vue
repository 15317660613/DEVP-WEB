<template>
  <div class="declareContent">
    ...
    <div class="uploadContent">
      <el-row>
        <el-col :span="12">
          ...
          <div class="uploadBtn">
            <div class="uploadSuccessTip" v-if="submitInfo.hydrogenFileName">
              <el-button @click="downFile(submitInfo.hydrogenFileId)">
              ...
              <el-upload
                ...
                :on-success="HySASupplyhandleSucess"
                :on-error="handleError"
              >
                <span>重新上传</span>
              </el-upload>
              <span @click="deleteFileHydrogen">删除</span>
            </div>
            <template v-if="!submitInfo.hydrogenFileName">
              <el-upload
                ref="upload"
                :action="action"
                :show-file-list="false"
              ...
              >
                <div>
                  <div class="upInfor">
                    ...
                    <span class="upName">核心材料上传</span>
                  </div>
                  <div class="upTooltip">支持扩展名：.pdf</div>
                </div>
              </el-upload>
            </template>
          </div>
        ...
        <el-col :span="12">
          ...
          <div class="uploadBtn">
            <div class="uploadSuccessTip" v-if="submitInfo.policyFileName">
              <el-button @click="downFile(submitInfo.policyFileId)">
              ...
              <el-upload
                ...
                accept=".pdf, .PDF"
                :before-upload="onBeforeUpload"
                :on-success="policyhandleSucess"
                :on-error="handleError"
              >
                <span>重新上传</span>
              </el-upload>
              <span @click="deleteFilePolicy">删除</span>
            </div>
            <template v-if="!submitInfo.policyFileName">
              <el-upload
                ref="upload"
                :action="action"
                :show-file-list="false"
        ...
        <div class="tableHandle">
          <el-form ref="ntvpForm" :model="ntvpForm" label-width="100px" class="form-inline">
            <el-form-item label="企业名称：">
              <el-select size="small" v-model="ntvpForm.tenantId">
                <el-option
            ...
            <el-form-item label="车辆种类：">
              <el-select size="small" v-model="ntvpForm.vecClass">
                <el-option
            ...
            <el-form-item label="车辆型号：">
              <el-input v-model="ntvpForm.model" size="small" placeholder="请填写"></el-input>
            </el-form-item>
            <el-form-item label="车辆VIN码：">
              <el-input v-model="ntvpForm.vehicle" size="small" placeholder="请填写"></el-input>
            </el-form-item>
            <el-button size="small" type="primary" class="search" @click="ntvpSearch">搜索</el-button>
            <el-button size="small" v-show="tab==1" class="handle" @click="addSettled">加入待清算</el-button>
            <el-button size="small" v-show="tab==2" class="handle" @click="cancelSettled">取消</el-button>
          </el-form>
        </div>
      </div>
      ...
      <div class="hscs_header">
        ...
        <div class="tableHandle">
          <el-form ref="hscsForm" :model="hscsForm" label-width="100px" class="form-inline">
            <el-form-item label="企业名称：">
              <el-select size="small" v-model="hscsForm.tenantId">
                <el-option
            ...
            <el-button type="primary" size="small" class="search" @click="hscsSearch">搜索</el-button>
          </el-form>
        </div>
      </div>
      ...
      <div class="spcy_header">
        ...
        <div class="tableHandle">
          <el-form ref="spcyForm" :model="spcyForm" label-width="100px" class="form-inline">
            <el-form-item label="企业名称：">
              <el-select size="small" v-model="spcyForm.tenantId">
                <el-option
            ...
            <el-button type="primary" size="small" class="search" @click="spcySearch">搜索</el-button>
          </el-form>
        </div>
      </div>
    ...
    <div class="declareFooter">
      <el-button size="small" class="subStorage" @click="subStorage">暂 存</el-button>
      <el-button size="small" class="submit" @click="submit">提 交</el-button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      hscsForm: {},
      spcyForm: {},
      action: config.API_CONFIG + "/upload?_t=" + new Date().getTime(), //上传接口
  ...
  methods: {
    ...
    onBeforeUpload(file) {
      ...
      const isLt1M = file.size < 10 * 1024 * 1024;
      if (!isIMAGE) {
    ...
    //氢能源上传成功
    HySASupplyhandleSucess(res) {
      this.submitInfo.hydrogenFileId = res.data.id;
      this.submitInfo.hydrogenFileName = res.data.name;
      this.loading.close();
    },
    deleteFileHydrogen() {
      this.submitInfo.hydrogenFileId = "";
      this.submitInfo.hydrogenFileName = "";
    },
    policyhandleSucess(res) {
      this.submitInfo.policyFileId = res.data.id;
      this.submitInfo.policyFileName = res.data.name;
      this.loading.close();
    },
    deleteFilePolicy() {
      this.submitInfo.policyFileId = "";
      this.submitInfo.policyFileName = "";
    },
    handleError(res) {
      console.log(res);
    },
    ...
    addSettled() {
      if (this.ntvpSelectData.length > 0) {
        let data = [];
        this.ntvpStorageData.forEach((item) => {
          this.ntvpSelectData.forEach((selectItem) => {
            if (item.vehicleId == selectItem.vehicleId) {
              data.push(item);
            }
          });
        });
        addAwaitSettlement(data).then((res) => {
          if (res.ok) {
    ...
    getVehicleList() {
      ...
      getVehicleList(params).then((res) => {
        this.ntvpStorageData = JSON.parse(JSON.stringify(res.data.data));
    ...
    getSite() {
      ...
      params = Object.assign(params, this.siteSearchCondition);
    ...
    getComponent() {
      ...
      params = Object.assign(params, this.partSearchCondition);
    ...
    submit() {
      let data = {
        hydrogenFileId: this.submitInfo.hydrogenFileId,
        policyFileId: this.submitInfo.policyFileId,
        provinceId: this.condition.provinceId,
        year: this.condition.year,
      };
